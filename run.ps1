...
# Set console output encoding to UTF-8 early to display box-drawing characters correctly
try {
...
function Load-Language {
    ...
    try {
        $response = (Invoke-WebRequest -Uri (Get-Link -e "/en.ps1") -UseBasicParsing).Content
        
        # Handle UTF-8 encoding properly for box-drawing characters
        # PowerShell's Invoke-WebRequest may return bytes or string depending on content type
        if ($response -is [byte[]]) {
        ...
        Write-Host "Error loading language file" -ForegroundColor Red
...
# Load language strings (English only)
$lang = Load-Language
